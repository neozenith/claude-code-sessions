-- Drill-down: Sessions for a specific project
-- Usage: Pass project_id as filter (handled in Python)
WITH pricing AS (
...
SELECT
    ...
    MIN(pd.timestamp_utc) AS start_time_utc,
    MIN(pd.timestamp_local) AS start_time_local,
    MAX(pd.timestamp_utc) AS end_time_utc,
    MAX(pd.timestamp_local) AS end_time_local,
    MAX(pd.timestamp_utc) - MIN(pd.timestamp_utc) AS duration,
...
GROUP BY pd.project_id, pd.session_id, pd.model_id
